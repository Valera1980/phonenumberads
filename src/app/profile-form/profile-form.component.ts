...
export class ProfileFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this._userService.queryGetMock().subscribe((prof) => {
  ...
  eventDeletPhone(phoneId: number | string, index: number): void {
    ...
    setTimeout(() => {
      ...
      this.isMain = this.isMain === phoneId ? null : this.isMain;
  ...
  getIsMain(phones: IPhoneNumber[]): number | string | null {
    ...
    if (!phones.find((p) => p.isMain === true)) {
      // если нет ни одного главного -  ставим первый
      return phones[0].id;
    }
  ...
  save(): void {
    if (this.isMain === null) {
      this.showToastMessage('Не выбран основной телефон!', 'error');
      return;
    }
